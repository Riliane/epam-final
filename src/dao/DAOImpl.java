...
public class DAOImpl {
    ...
    public List<BorrowRecord> getBorrows(String id, String table, String parameter) throws SQLException, ClassNotFoundException {
        Statement st = null;
        ResultSet rs = null;
        Class.forName("org.gjt.mm.mysql.Driver");
        if (con == null) {
        ...
        if (parameter.equals("documents")) {
            query = "SELECT " + table + ".document, documents.name, documents.author, " + table + ".date_of_borrow, " + table + ".date_of_return FROM documents INNER JOIN " + table + " ON documents.document_id = " + table + ".document WHERE " + table + ".reader = " + id;
        } else if (parameter.equals("readers")) {
            query = "SELECT " + table + ".reader, readers.first_name, readers.last_name, " + table + ".date_of_borrow, " + table + ".date_of_return FROM readers INNER JOIN " + table + " ON readers.reader_id = " + table + ".reader WHERE " + table + ".document = " + id;
        }
        logger.info("Query: " + query);
        rs = st.executeQuery(query);
        List<BorrowRecord> list = new BorrowListCreator(parameter).createResultList(rs);
        return list;
    }
    ...
    public void insertLibrarian(String username, String password) throws SQLException, ClassNotFoundException, NoSuchAlgorithmException {
        insertUser(username, password, "librarian");
    }
    ...
    public void returnDocument(int id) throws SQLException, ClassNotFoundException {
        updateDocument(id, "return");
    }
